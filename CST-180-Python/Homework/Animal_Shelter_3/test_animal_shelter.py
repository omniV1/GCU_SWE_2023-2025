...
import unittest
from animal_shelter import Bird, Cat, Dog, Kennel, Shelter
...
# SHELTER CLASS TESTS
# ====================================================================================
# These tests verify the new Shelter object introduced in project part 3.
# They validate capacity enforcement, kennel reuse, adoption, and the waitlist.
# ====================================================================================

class TestShelter(unittest.TestCase):
    """Test cases for the Shelter class."""

    def test_shelter_capacity_must_be_positive(self):
        """Shelter constructor should reject zero or negative capacity values."""
    ...
    def test_shelter_reuses_empty_kennel_before_creating_new(self):
        """Animals should occupy empty kennels before creating new ones."""
        shelter = Shelter(2)
        dog = Dog("Buddy", 3, "Labrador")
        cat = Cat("Mittens", 2, "Tabby")
        bird = Bird("Sky", 1, 0.35)

        self.assertTrue(shelter.add_animal(dog))
        self.assertTrue(shelter.add_animal(cat))
        adopted_cat = shelter.adopt_animal("Cat")
        self.assertIsNotNone(adopted_cat)
        self.assertTrue(shelter.has_empty_kennel())
        self.assertTrue(shelter.add_animal(bird))
        self.assertEqual(shelter.total_animals(), 2)

    def test_shelter_enforces_capacity_limit(self):
        """Adding animals beyond the capacity should fail."""
        shelter = Shelter(2)
        self.assertTrue(shelter.add_animal(Dog("Rex", 4, "Beagle")))
        self.assertTrue(shelter.add_animal(Cat("Luna", 5, "Orange")))
        self.assertFalse(shelter.add_animal(Bird("Sky", 1, 0.35)))

    def test_shelter_adopt_removes_animal(self):
        """Adopting an animal should remove it from its kennel."""
        shelter = Shelter(2)
        dog = Dog("Buddy", 3, "Labrador")
        shelter.add_animal(dog)
        adopted_dog = shelter.adopt_animal("Dog")
        self.assertEqual(adopted_dog, dog)
        self.assertTrue(shelter.has_empty_kennel())
        self.assertEqual(shelter.total_animals(), 0)

    def test_waitlist_records_missing_animal_requests(self):
        """If requested animal type is unavailable, adopter is added to waitlist."""
        shelter = Shelter(1)
        result = shelter.adopt_animal("Rabbit", adopter_name="Alice")
        self.assertIsNone(result)
        self.assertEqual(shelter.get_waitlist("Rabbit"), ["Alice"])
        # Ensure retrieving waitlist does not allow external mutation
        returned_waitlist = shelter.get_waitlist("Rabbit")
        returned_waitlist.append("Bob")
        self.assertEqual(shelter.get_waitlist("Rabbit"), ["Alice"])
...
# - 5 tests for the Shelter class (capacity, kennel reuse, adoption, waitlist)
#
# Total: 33 tests covering all project requirements
# ====================================================================================
